 ...
 *
 *   planning   : research[architecture|risks|libraries] ∥  →  planner  ⇒ план/ТЗ
...
class CodingPipeline(private val client: ChatClient) {
    ...
    fun runPlanning(request: String, profileBlock: String, feedback: String): StageResult {
        val brief = brief("Задача пользователя", request, feedback)
        val researchers = listOf(
            agent("research:architecture", "архитектура", RESEARCH_ARCH),
            agent("research:risks", "риски и крайние случаи", RESEARCH_RISKS),
            agent("research:libraries", "библиотеки и API", RESEARCH_LIBS),
        )
        // Рой исследователей работает параллельно — каждый по своему направлению.
        val research = Swarm.run(researchers.map { a -> { a.run(brief, profileBlock) } })

        val researchDigest = research.joinToString("\n\n") { "### ${it.role} (${it.name})\n${it.output}" }
        val planner = agent("planner", "синтез плана/ТЗ", PLANNER)
        val plan = planner.run(
            "$brief\n\nРезультаты исследования:\n$researchDigest",
            profileBlock,
        )
        return StageResult(TaskStage.PLANNING, plan.output, research + plan)
    }
    ...
    private companion object {
        ...
        const val RESEARCH_LIBS =
            "Ты research-агент. Направление: БИБЛИОТЕКИ И API. Предложи подходящие библиотеки/встроенные " +
                "средства языка из профиля, их плюсы/минусы и что выбрать. Тезисно, без кода."
        const val PLANNER =
            "Ты ведущий планировщик. Составь короткий план-ТЗ под ИМЕННО эту задачу, в объёме MVP. " +
                "НЕ раздувай скоуп и НЕ добавляй нефункциональные требования (огромные файлы, " +
                "производительность, масштабирование, обязательные внешние библиотеки), которых " +
                "пользователь не просил. Дай нумерованные шаги, ключевые решения, список файлов/функций " +
                "и РЕАЛИСТИЧНЫЕ критерии готовности, достижимые за один проход. Конкретно, без кода."
        const val CODER =
            "Ты coder-агент. Реализуй задачу строго по плану-ТЗ и профилю. ВАЖНО: соблюдай ключевые " +
