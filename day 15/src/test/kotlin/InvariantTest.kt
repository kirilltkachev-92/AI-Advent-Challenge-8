...
class InvariantTest {
    ...
    @Test
    fun `clean request is accepted and starts the lifecycle at clarify`() {
        val agent = tempAgent(InvFakeClient())
        val outcome = agent.startTask("Сделай экран профиля")
        assertTrue(outcome is StageOutcome.Ran)
        assertEquals(TaskStage.CLARIFY, (outcome as StageOutcome.Ran).stage)
    }
    ...
    fun `code that violates an invariant surfaces a warning after fix attempts`() {
        ...
        val agent = tempAgent(client)
        agent.startTask("Сделай поток событий")     // clarify (запрос чистый — задача стартует)
