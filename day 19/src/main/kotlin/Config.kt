...
/**
 * Конфигурация Дня 19 (композиция MCP-инструментов).
 *
 * Источники (по приоритету): переменные окружения → .env → дефолт.
 * Википедия бесплатна и без ключа; DEEPSEEK_API_KEY нужен агенту, который выстраивает
 * цепочку вызовов инструментов из запроса пользователя.
 */
object Config {
    ...
    fun apiKey(): String =
        envValue(API_KEY_VAR)
            ?: error("Нет API-ключа. Задайте $API_KEY_VAR в окружении или .env (можно из «day 18»/«day 1»).")
    ...
    /** Каталог, куда инструмент save_to_file пишет файлы. */
    ...
    /** Минимальная пауза между запросами к Википедии, мс (вежливый троттлинг от бана). */
