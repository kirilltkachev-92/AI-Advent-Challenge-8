...
class CodingPipelineTest {
    ...
    @Test
    fun `clarify runs an analyst`() {
        val client = RecordingClient()
        val r = CodingPipeline(client).runClarify("сделай экран", "профиль", "")
        assertEquals(TaskStage.CLARIFY, r.stage)
        assertEquals(1, r.runs.size)
        assertEquals("analyst", r.runs.first().name)
    }

    @Test
    fun `planning runs a council, a debate round and an orchestrator`() {
        val client = RecordingClient()
        val r = CodingPipeline(client).runPlanning("сделай экран", "требования", "профиль", "")
        assertEquals(TaskStage.PLANNING, r.stage)
        assertEquals(9, r.runs.size) // 4 предложения + 4 дебата + оркестратор
        assertEquals(9, client.calls)
        assertTrue(r.runs.map { it.name }.containsAll(
            listOf(
                "proposer:architecture", "proposer:risks", "proposer:libraries", "proposer:ui-structure",
                "debater:architecture", "debater:risks", "debater:libraries", "debater:ui-structure",
                "orchestrator:plan",
            ),
        ))
    }
    ...
    fun `profile block is injected into agents`() {
        val client = RecordingClient()
        CodingPipeline(client).runPlanning("сделай парсер", "требования", "[Профиль разработчика] Язык: Rust", "")
    ...
    @Test
    fun `startTask runs clarify and produces requirements`() {
        val agent = tempAgent(RecordingClient())
        val out = agent.startTask("сделай CLI")
        assertTrue(out is StageOutcome.Ran)
        assertEquals(TaskStage.CLARIFY, (out as StageOutcome.Ran).stage)
        assertNotNull(agent.task.artifact(TaskStage.CLARIFY))
    }

    @Test
    fun `advance runs the next stage and illegal jump is blocked`() {
        val agent = tempAgent(RecordingClient())
        agent.startTask("сделай CLI")     // clarify
        val adv = agent.advance()         // clarify → planning
        assertTrue(adv is StageOutcome.Ran)
        assertEquals(TaskStage.PLANNING, agent.task.stage)
        assertNotNull(agent.task.artifact(TaskStage.PLANNING))

        val blocked = agent.gotoStage("done") // planning → done запрещён
    ...
    fun `task survives a restart at any stage (pause and resume)`() {
        ...
        val agent = newAgent()
        agent.startTask("сделай CLI") // clarify
        agent.advance()              // → planning
        assertEquals(TaskStage.PLANNING, agent.task.stage)

        // «Перезапуск»: новый агент с тем же хранилищем.
        val reloaded = newAgent()
        assertEquals(TaskStage.PLANNING, reloaded.task.stage)
        assertEquals("сделай CLI", reloaded.task.request)
        assertNotNull(reloaded.task.artifact(TaskStage.CLARIFY))
