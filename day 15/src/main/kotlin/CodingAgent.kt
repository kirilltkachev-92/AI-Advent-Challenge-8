...
class CodingAgent(
    ...
    /** Новая задача: проверяем запрос на инварианты, затем стартуем с первого этапа (clarify). */
    fun startTask(request: String): StageOutcome {
        ...
        task.start(request)
        val run = runStage(task.stage, feedback = "") // task.stage == TaskStage.INITIAL (clarify)
    ...
    private fun produce(stage: TaskStage, req: String, ctx: String, feedback: String): StageResult =
        when (stage) {
            TaskStage.CLARIFY -> pipeline.runClarify(req, ctx, feedback)
            TaskStage.PLANNING ->
                pipeline.runPlanning(req, task.artifact(TaskStage.CLARIFY) ?: "", ctx, feedback)
            TaskStage.EXECUTION ->
