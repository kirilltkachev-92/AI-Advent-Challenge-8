...
class CodingPipelineTest {
    ...
    private fun tempAgent(client: ChatClient): CodingAgent {
        val root = createTempDirectory("day13")
        return CodingAgent(
            client = client,
    ...
    fun `task survives a restart at any stage (pause and resume)`() {
        val root = createTempDirectory("day13-resume")
        val client = RecordingClient()
        val profiles = ProfileStore(root.resolve("profiles"))
        val tasks = { TaskStore(root.resolve("state")) }

        val agent = CodingAgent(client, profiles, tasks(), CodingPipeline(client))
        agent.startTask("сделай CLI")
        agent.advance() // на execution
        assertEquals(TaskStage.EXECUTION, agent.task.stage)

        // «Перезапуск»: новый агент с тем же хранилищем.
        val reloaded = CodingAgent(client, ProfileStore(root.resolve("profiles")), tasks(), CodingPipeline(client))
