...
/**
 * Конфигурация Дня 22 (первый RAG-запрос).
 *
 * Эмбеддинги и поиск — локальная Ollama (nomic-embed-text), как в Дне 21.
 * DEEPSEEK_API_KEY обязателен: он нужен самому агенту (оба режима — с RAG и без)
 * и судье в сравнении качества.
 */
object Config {
    ...
    // Для RAG достаём чуть больше контекста, чем в поиске Дня 21.
    private const val DEFAULT_TOP_K = 4

    // День 21 показал: на большой статье структурная стратегия точнее (4:2 у судьи).
    private const val DEFAULT_STRATEGY = "structure"

    // Секции длиннее лимита дорезаются окном — контекст nomic-embed-text ограничен.
    ...
    /** Какой индекс использует RAG: structure (по умолчанию) или fixed. */
