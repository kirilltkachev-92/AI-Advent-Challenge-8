...
/**
 * Строит (или загружает из кеша) индекс базы знаний продукта.
 * Кеш по SHA-256 корпуса — как в Днях 28–32: пересчёт эмбеддингов только
 * когда FAQ или документация реально поменялись.
 */
object Indexer {
    fun buildOrLoad(embedder: OllamaEmbedder): DocumentIndex {
        val docs = KnowledgeBase.collect(Config.docsDir())
        check(docs.isNotEmpty()) { "Не найдено ни одного .md в ${Config.docsDir()}" }
        val sha = KnowledgeBase.corpusSha(docs)
        ...
        val chunks = docs.flatMap { KnowledgeBase.chunk(it, Config.maxSectionChars()) }
        ...
        val index = DocumentIndex(
            ...
            strategy = KnowledgeBase.STRATEGY,
            corpus_sha = sha,
